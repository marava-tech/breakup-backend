...
public class StoryDataStoreService {
    ...
    public StorySearchResponse comprehensiveSearch(StorySearchRequest request, Authentication authentication) {
        ...
        try {
            ...
            if (request.hasTextSearch()) {
                ...
                if (request.getMinEmotionScore() != null) {
                    ...
                    appliedFilters.add("emotion: " + request.getEmotionType() + " (min score: "
                            + request.getMinEmotionScore() + ")");
                } else {
                ...
                // User search
                searchResults = dataStoreRepository.findByUserIdAndProcessingStatus(request.getUserId(),
                        convertToProcessingStatus(request.getStatus()));
                ...
                // Language search
                searchResults = dataStoreRepository.findByLanguageAndProcessingStatus(request.getLanguage(),
                        convertToProcessingStatus(request.getStatus()));
                ...
                if (request.getCreatedAfter() != null && request.getCreatedBefore() != null) {
                    searchResults = dataStoreRepository.findByCreatedAtBetween(request.getCreatedAfter(),
                            request.getCreatedBefore());
                    appliedFilters
                            .add("date_range: " + request.getCreatedAfter() + " to " + request.getCreatedBefore());
                ...
                searchResults = dataStoreRepository
                        .findByProcessingStatus(convertToProcessingStatus(request.getStatus()));
        ...
        // Sort stories by affinity score (Play count + Completion count weighting +
        // User prefs)
        stories.sort((s1, s2) -> Double.compare(
        ...
        log.info("Comprehensive search completed in {}ms. Found {} results", searchTime,
                paginatedStoryResponses.size());
    ...
    private StoryResponse convertToStoryResponse(Story story, String currentUserId) {
        ...
        // Check if current user liked/bookmarked this story (only if currentUserId is
        // provided)
