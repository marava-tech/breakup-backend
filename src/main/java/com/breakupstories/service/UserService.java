...
public class UserService implements UserDetailsService {
    ...
    public UserResponse createUserAfterOtpVerification(UserRequest request) {
        ...
        // !request.getReferralCode().trim().isEmpty()) {
        // String deviceId = request.getDeviceId();
        // if (deviceId != null && !deviceId.trim().isEmpty()) {
        // rewardService.processReferral(savedUser.getId(),
        // request.getReferralCode().toUpperCase(), deviceId);
        // } else {
        // log.warn("Referral code provided but no device ID, skipping referral
        // processing for user: {}", savedUser.getId());
        // }
        // }
    ...
    public UserProfileResponse getUserProfileWithStatistics(String userId) {
        ...
        Long totalStories = storyRepository.countByUserIdAndStatus(userId,
                com.breakupstories.model.Story.StoryStatus.ACTIVE);

        // Get all story IDs for this user to calculate likes and comments
        List<String> userStoryIds = storyRepository
                .findByUserIdAndStatus(userId, com.breakupstories.model.Story.StoryStatus.ACTIVE)
                .stream()
